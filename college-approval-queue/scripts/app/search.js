/* scripts/app/search.js — module split from former scripts/app.js (one IIFE, now dissolved to shared global scope). Advanced search modal + results view, top free-text search, Row-3 toolbar wiring, workspace search banner.
  ...
  function buildUnifiedAppPool() {
    var pool = [];
    // With institution review off the college owes no decision, so these apps live in
    // Waiting (see reviewBucket/waitingBucket in core.js) — Global Search has to file
    // them the same way, or it would return as "Needs Review" records the queue has
    // already moved on. Mirrors the HS fork's counselorApproval handling.
    var _reviewReq = (window.DENetwork ? window.DENetwork.get('institutionReview') : true) !== false;
    ALL_APPS.forEach(function(a) {
      if (_reviewReq) pool.push(Object.assign({}, a, { status: 'pending', _receivedRaw: a.submitted }));
      else pool.push(Object.assign({}, a, { status: 'waiting', awaitingConsent: false, awaitingCounselor: false, awaitingRegistration: true, _receivedRaw: a.submitted }));
    });
    ...
    ALL_ADMITTED_APPS.forEach(function(a) {
      pool.push(Object.assign({}, a, { status: 'admitted', _receivedRaw: a.submitted }));
    });
  ...
  /* ─── What Global Search may offer, given the network config ───
     ...
     interchangeable — this is the matrix:
       ...
       Institution                     fork shape                   hidden (WVCC only)
       Status "Needs Review"           this org's APPROVAL GATE     institutionReview
       Status "Invited"                this org's INVITE entry pt   heInvite
       Status Waiting/Admitted/Registered/     this org's VISIBILITY        heView
  ...
  function searchableStatuses() {
    ...
    var allowed = [];
    if (N.get('institutionReview') !== false) allowed.push('pending');
    if (N.canInvite('he')) allowed.push('invited');
    if (N.canView('he')) allowed.push('waiting', 'admitted', 'active', 'denied', 'cancelled');
    return allowed;
  }
  ...
  const STATUS_LABEL = { pending: 'Needs Review', admitted: 'Admitted', waiting: 'Waiting', invited: 'Invited', active: 'Registered', denied: 'Denied', cancelled: 'Cancelled' };
  const STATUS_BADGE_CLASS = { pending: 'is-solid is-note', admitted: 'is-solid is-primary', waiting: 'is-solid is-primary', invited: 'is-solid is-primary', active: 'is-solid is-success', denied: 'is-solid is-error', cancelled: '' };
  const FLAG_LABEL = {
  ...
  function renderAdvancedResults() {
    ...
    pg.slice.forEach(function(a) {
      ...
      // High school has its own column now, so no name sub-line is needed.
      var subLine = '';
  ...
  function jumpToFirstMatchingSegment() {
    ...
    var order = ['needs-review', 'admitted', 'waiting', 'active', 'denied'];
  ...
  /* Status text that matches the table's tag: the shared detailStatusLabel() resolves the
     admitted/waiting/closed sub-status; the segment key maps to the row's lifecycle status. */
  function deStatusText(a, segKey) {
    var status = segKey === 'needs-review' ? 'pending'
               : segKey === 'active'       ? 'active'
               : segKey === 'admitted'     ? 'admitted'
  ...
  function deSegmentRows(segKey) {
    ...
    if (segKey === 'needs-review') {
      rows = activeApps.filter(function(a) {
      ...
      var base = segKey === 'waiting'  ? WAITING_APPS
               : segKey === 'admitted' ? ALL_ADMITTED_APPS
               : segKey === 'active'   ? ALL_ACTIVE_APPS
               : ALL_DENIED_APPS;
      rows = base.filter(function(a) { return !t || appHaystack(a).indexOf(t) !== -1; });
    }
    return rows;
  }
  var SEG_FILE_LABEL = { 'needs-review': 'needs-review', 'waiting': 'waiting', 'admitted': 'admitted', 'active': 'registered', 'denied': 'closed' };
