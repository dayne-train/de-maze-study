/* scripts/app/confirm.js — module split from former scripts/app.js (one IIFE, now dissolved to shared global scope). Deny screen, confirmEndorse, approve-success screen, confirmDeny.
  ...
  function confirmEndorse(ids) {
    // Single-learner approvals route through the Admit workflow page (Review Summary and Confirm).
    if (ids.length === 1 && typeof requestApproveConsent === 'function') {
      var gateApp = activeApps.find(function(a) { return a.id === ids[0]; });
      if (gateApp) {
        requestApproveConsent(gateApp.id, gateApp.institution, gateApp.firstName + ' ' + gateApp.lastName,
  ...
  function doEndorse(ids) {
    ...
    if (fromReview) {
      ...
      var msg = n === 1
        ? 'Application approved — Institution Review complete.'
        : n + ' applications approved — Institution Review complete.';
  ...
  function doEndorseHold(ids) {
    ...
    refreshUnifiedPool();
    if (fromReview) {
      transitionReviewInPlace(ids[0], 'admitted');
      var _f = findAppAnywhere(ids[0]);
      showToast(_f ? _f.app.firstName + ' ' + _f.app.lastName + ' admitted, on hold until invited to register.' : 'Learner admitted, on hold.', 'success');
    } else {
      var msg = n === 1
        ? 'Learner admitted, on hold until invited to register.'
        : n + ' learners admitted, on hold until invited to register.';
      showToast(msg, 'success');
    }
  }
