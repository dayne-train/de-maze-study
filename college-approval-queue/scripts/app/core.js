/* scripts/app/core.js — module split from former scripts/app.js (one IIFE, now dissolved to shared global scope). Shared state (selectedIds, activeApps, searchTerm…), paginator utility + instances, exchange model, step dots.
  ...
  /* ─── Exchange-network buckets (shared DENetwork model) ───
     ...
     is instant and the approve/deny paths stay untouched. Translated apps are marked
     _xlated so nothing writes them back. Mirrors the HS fork's counselorApproval
     handling in hs-approval-queue/scripts/app/core.js. */
  function _netGate(k) { return (window.DENetwork ? window.DENetwork.get(k) : true) !== false; }
  function _reviewToWaiting(a) {
    return Object.assign({}, a, {
      awaitingConsent: false, awaitingCounselor: false,
      awaitingRegistration: true, _xlated: true
    });
  }
  function reviewBucket()  { return _netGate('institutionReview') ? activeApps : []; }
  function waitingBucket() {
    return _netGate('institutionReview')
      ? WAITING_APPS
  ...
  let selectedAdmittedIds = new Set();
  ...
  function emptyAdvCriteria() {
    return {
      ...
      institution: '', group: '',
      statuses: new Set(),       // 'pending' | 'admitted' | 'waiting' | 'invited' | 'active' | 'denied' | 'cancelled'
  ...
  /* (bulkAlignment A/B retired Jun 30 — college-admin has one institution, no layout variance.) */
  ...
  /* ─── Entry points ───
     Which initiation points a network offers lives in the shared DENetwork model as
     four independent switches (heInvite · hsInvite · selfUrl · dashboard), replacing
     the old invite/open/combined enum. Read via DENetwork.canInvite()/canSelfApply().

     The enum also doubled as the driver for the High School column, which conflated
     two unrelated things: how a learner starts an application, and how many high
     schools this college serves. They're now separate — multiHs below — and since
     WVCC is one college to many high schools, this defaults ON here (the mirror of
     the HS fork, where one counselor normally sees one school and it defaults OFF). */
  var multiHs = true;
  ...
  function applyExchangeModel() {
    // Which initiation features exist in each model:
    //   invite-only → the college invites learners; no open/self-serve applications.
    //   open        → learners self-apply; no inviting capability.
    //   combined    → both are active.
    //
    // College review is NOT an initiation feature. It is the college's own approval
    ...
    // Inviting is this college's own capability, independent of whether its partner
    // high schools may invite.
    var hasInvite = window.DENetwork ? window.DENetwork.canInvite('he') : true;

    // ── Workspace offerings ──
    // "Review Applications" is always offered; "Open" has no "Invite" tile.
    ...
    var hasView = window.DENetwork ? window.DENetwork.canView('he') : true;
    ['seg-btn-waiting', 'seg-btn-admitted', 'seg-btn-active', 'seg-btn-denied'].forEach(function (id) {
  ...
  var admittedPag  = makePaginator('admitted-results-label',    'admitted-page-buttons',    'admitted-perpage-select',    function() { renderAdmittedTable(); });
