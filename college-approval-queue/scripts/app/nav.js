/* scripts/app/nav.js — module split from former scripts/app.js (one IIFE, now dissolved to shared global scope). showScreen navigation, transcript viewer modal, tracker hover popover.
  ...
  function showScreen(name, param) {
    ...
    } else if (name === 'admit-confirm') {
      // currentActionIds/currentActionSource are set by requestApproveConsent before navigating here.
      renderAdmitConfirmScreen();
      var fromReviewAdmit = currentActionSource === 'review';
      var admitBackBtn = document.getElementById('admit-confirm-back-btn');
      if (admitBackBtn) {
        admitBackBtn.onclick = function() {
          if (fromReviewAdmit) showScreen('review', currentReviewId);
          else showScreen('de');
        };
        admitBackBtn.innerHTML = '<i class="ti ti-chevron-left"></i> ' + (fromReviewAdmit ? 'Back' : 'Back to Queue');
      }
    }
  }
  ...
  function buildTrackerPopoverHTML(app, status) {
    ...
    if (status === 'invited') {
      ...
      // College view: College Review is done (our step); the active gates are
      // the counselor and/or guardian consent.
      steps = [
        ...
        { label: 'High School Approval', state: app.awaitingCounselor ? 'active' : 'done', sub: app.awaitingCounselor ? 'Awaiting approval' : 'Approved', subCls: app.awaitingCounselor ? 'active' : 'done' },
      ...
      steps = [
        ...
        { label: 'Institution Review',     state: 'done', sub: 'Approved',  subCls: 'done' },
        { label: 'Registered',         state: 'done', sub: app.enrolledDate || 'Registered', subCls: 'done' },
      ];
    } else if (status === 'denied') {
      var deniedStepMap = { 'Application': 'Application Submission', 'Guardian Consent': 'Parent/Guardian Consent', 'Counselor Review': 'High School Approval', 'Institution Review': 'Institution Review', 'College Review': 'Institution Review' };
      var deniedStep = deniedStepMap[app.deniedState] || 'Institution Review';
      var before = true;
    ...
    var steps_pending = [
      ...
      { label: 'High School Approval', state: 'done',                       sub: 'Approved',              subCls: 'done'   },
      { label: 'Institution Review',     state: 'active',                     sub: 'Awaiting your review',  subCls: 'active' },
    ];
    steps = steps_pending;
    }
  ...
  wireTrackerHover('queue-table',   function(id) { return activeApps.find(function(a) { return a.id === id; }); },        'pending');
  wireTrackerHover('waiting-table', function(id) { return WAITING_APPS.find(function(a) { return a.id === id; }); },     'waiting');
