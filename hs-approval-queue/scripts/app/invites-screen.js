/* scripts/app/invites-screen.js — the Invites top-level tab (Corey review #7: an invite
  ...
  function renderInvitedTable() {
    ...
    pg.slice.forEach(function(e) {
      ...
      rows += nameCells(e);
      rows += '<td style="font-size:13px;">' + e.college + '</td>';
      rows += '<td class="col-hs" style="font-size:13px;">' + (e.school || '—') + '</td>';
  ...
  function exportInvited() {
    ...
    var header = ['Last Name', 'First Name', 'Institution', 'High School', 'Group', 'Date Invited', 'Last Sent', 'Status'];
    var lines = [header.map(esc).join(',')];
    rows.forEach(function(e) {
      lines.push([e.lastName, e.firstName, e.college, (e.school || ''), e.group, e.dateInvited, (e.lastSent || e.dateInvited), 'Invited'].map(esc).join(','));
    });
