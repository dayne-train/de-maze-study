/* scripts/app/review.js — module split from former scripts/app.js (one IIFE, now dissolved to shared global scope). Single-application review screen renderer.
  ...
  var DETAIL_STATUS_LABEL = { pending:'Needs Review', invited:'Invited', waiting:'Waiting', active:'Registered', denied:'Denied', cancelled:'Cancelled' };
  var DETAIL_STATUS_BADGE = { pending:'is-solid is-note', invited:'is-solid is-primary', waiting:'is-solid is-primary', active:'is-solid is-success', denied:'is-solid is-error', cancelled:'is-solid is-bold' };
  ...
  function findAppAnywhere(id) {
    var a;
    // reviewBucket()/waitingBucket() reflect the gate: with counselor approval off,
    // a Needs Review app resolves as a (translated) Waiting app instead.
    a = reviewBucket().find(function(x){return x.id===id;});   if (a) return { app:a, status:'pending' };
    a = waitingBucket().find(function(x){return x.id===id;});  if (a) return { app:a, status:'waiting' };
  ...
  function detailDataFor(app) {
    ...
    var d = {
      ...
      counselor: app.counselor || COUNSELORS[currentCounselor].name,
      school: app.school || 'Pioneer High School',
      dateSent: app.dateInvited || app.submitted || app.enrolledDate || app.deniedDate || '\u2014',
      learnerEmail: (app.firstName.charAt(0) + app.lastName).toLowerCase().replace(/[^a-z]/g,'') + '@pioneerhs.edu',
  ...
  /* Exchange-network gates (shared DENetwork model). A network can switch off the
     guardian-consent (idx 2), counselor-approval (idx 3) and/or institution-review
     ...
     Defaults ON. With all three off only Invitation → Submission → Registration
     survive: the learner registers straight off submit (KB §8). */
  ...
  /* Which of the 6 detail steps a closed application died on.
       2 Parent/Guardian Consent · 3 High School Approval · 4 Institution Review */
  function _deniedStepIdx(app) {
    return (app.deniedState === 'Guardian Consent')   ? 2
         : (app.deniedState === 'Institution Review') ? 4
         : 3;
  }
  ...
  function stepperStatesFor(app, status) {
    ...
    if (status === 'invited') { s[0]='complete'; s[1]='current'; }
      ...
      s[2] = app.hasAlert ? 'current' : 'complete';
      s[3]='current';
    }
    else if (status === 'waiting') {
      s[0]='complete'; s[1]='complete'; s[3]='complete';
      s[2] = app.awaitingConsent ? 'current' : 'complete';
      if (app.awaitingInstitution) s[4]='current';
  ...
  function renderReviewScreen(id) {
    ...
    var stepSubs = [ d.institution, learnerName, d.guardianName, d.counselor, 'Admissions Team', learnerName ];
    ...
    if (status === 'pending') {
      actions =
        '<div class="appdetail-actions">' +
          '<button class="tasty-btn is-success is-md appdetail-action-btn" id="detail-approve-btn">Approve</button>' +
      ...
      // Three ordered, mutually exclusive sub-states: awaiting consent → (gates cleared)
      // awaiting institution review → (institution done) awaiting the learner to complete
      // course registration. Offer an optional nudge to whichever party we're blocked on.
      var waitMsg = app.awaitingRegistration ? 'Waiting on the learner to register for their courses.'
                  : app.awaitingConsent      ? 'Waiting on the parent/guardian to provide consent.'
                  :                            'Waiting on the institution to complete its review.';
      var remindLabel = app.awaitingRegistration ? 'Send reminder to learner'
                      : app.awaitingConsent      ? 'Send reminder to guardian'
                      :                            'Send reminder to institution';
      var remindParty = app.awaitingRegistration ? 'the learner'
                      : app.awaitingConsent      ? 'the parent/guardian'
                      :                            'the institution';
      actions = '<div class="appdetail-actions">' +
          '<button class="tasty-btn is-bold is-ghost is-md is-full" id="detail-remind-btn" data-party="' + remindParty + '"><i class="ti ti-bell"></i> ' + remindLabel + '</button>' +
  ...
  function transitionReviewInPlace(id, type) {
    ...
    setTimeout(function() {
      ...
      if (type === 'approved') {
        var waitMsg = app.awaitingRegistration ? 'Waiting on the learner to register for their courses.'
          : app.awaitingConsent  ? 'Waiting on guardian consent.'
          : app.awaitingInstitution ? 'Waiting on institution review.'
          : 'Approved \u2014 moving to registration.';
