/* scripts/app/boot.js — module split from former scripts/app.js (one IIFE, now dissolved to shared global scope). Denial-reason modal, Tweaks bridge (window.__dev), the showScreen invite-extension (relocated here so showScreen is already defined), and the initial render. Loaded LAST.
  ...
  // Nudge a party we're blocked on (guardian consent / institution review) from the
  // application detail view. Party string comes from the button (e.g. "the parent/guardian").
  ...
  window.__dev = {
    ...
    setPersona: function(key) {
      currentCounselor = key;
      var c = COUNSELORS[key];
      if (!c) return;
      var initials = (c.school || '').split(/\s+/)
        .map(function(w) { return w[0]; }).join('').slice(0, 3).toUpperCase() || 'PHS';
    ...
    setGate: function(key, on) {
      ...
      renderActiveTable();
      renderDeniedTable();
      // Counselor approval off ⇒ no counselor review ⇒ Needs Review tab/card hide
      // (folded into applyExchangeModel's hasReview, same path as invite-only).
      applyExchangeModel();
    ...
    setActiveEnrollments: function(on) {
      showActiveEnrollments = !!on;
      applyActiveEnrollmentsScope();
    },
  ...
  renderDeniedTable();
  applyExchangeModel();
  applyHsScope();   // HS column hidden by default (single high school)
  applyActiveEnrollmentsScope();   // Active Enrollments tab hidden by default (concept, dev-drawer only)
