...
(function () {
  ...
  function init() {
    ...
    DevDrawer.build('Prototype Settings', function (p) {
      ...
      p.toggle('See all applications', true, function (on) { view('hsView', on); });
      p.note('High schools');
      p.segmented([['1 HS', 'single'], ['Multiple', 'multiple']], 'single', function (v) { d().setMultiHs && d().setMultiHs(v === 'multiple'); });

      p.section('Scenario');
      p.note('Queue size');
      p.grid([['0', '0'], ['3', '3'], ['8', '8'], ['20', '20']], function (v) { d().setQueueSize && d().setQueueSize(v); }, { active: '8', columns: 2 });
      ...
      p.note('Persona');
      p.grid([['Morgan Lee', 'morgan'], ['James Park', 'james']], function (v) { d().setPersona && d().setPersona(v); }, { active: 'morgan' });
      p.note('Theme');
      ...
      p.toggle('Grades (concept)', false, function (v) { d().setActiveEnrollments && d().setActiveEnrollments(v); });
      p.toggle('Show In Progress (demo)', true, function (v) { window.setShowInProgress && setShowInProgress(v); });
      p.segmented([['A · Table', 'table'], ['B · Split View', 'split'], ['C · Report', 'reportc']], 'table', function (v) { window.setActiveEnrollmentsVariant && setActiveEnrollmentsVariant(v); });
