/* scripts/app/queue.js — module split from former scripts/app.js (one IIFE, now dissolved to shared global scope). Needs-Review queue: table render, quick-filters/sort, badges, row event delegation, move helpers, approveFromQueue.
  ...
  function renderTable() {
    ...
    visibleApps = reviewBucket().filter(function(app) {
    ...
    if (visibleApps.length === 0) {
      ...
      tbody.innerHTML = '<tr><td colspan="10"><div class="empty-state"><i class="ti ti-circle-check"></i>' + emptyMsg + '</div></td></tr>';
      queuePag.renderControls(0, 1, 0, 0, reviewBucket().length);
      updateBadges();
      return;
    }
    ...
    tbody.innerHTML = pg.slice.map((app, i) => {
      ...
      return `
        <tr class="${rowCls}" data-id="${app.id}">
          ...
          <td style="font-size:13px;">${COLLEGES[app.institution] || app.institution}</td>
          <td class="col-hs" style="font-size:13px;">${app.school || '—'}</td>
          ...
          <td class="col-actions">
            <div class="row-actions">
              ${viewAppBtn(app.id)}
              <button class="tasty-btn is-success is-sm" data-id="${app.id}" data-action="approve">Approve</button>
            </div>
          </td>
        </tr>`;
    }).join('');

    queuePag.renderControls(pg.total, pg.pages, pg.start, pg.end, reviewBucket().length);
    updateBadges();
    updateBulkBar();
  ...
  /* ─── Move helpers — relocate an app between status tabs ───
     ...
     done, now awaiting consent/institution). Denying sends it to Closed. */
  function todayLabel() {
  ...
  function moveAppToWaiting(app) {
    ...
    var copy = Object.assign({}, app, {
      ...
      awaitingInstitution: !app.hasAlert
    });
  ...
  function moveAppToDenied(app, reason) {
    ...
    var copy = Object.assign({}, app, {
      ...
      deniedBy: (COUNSELORS[currentCounselor] && COUNSELORS[currentCounselor].name) || 'Counselor',
      deniedState: 'Counselor Review',
  ...
  function approveFromQueue(id) {
    ...
    if (!app) return;
    // First-approval consent gate (until dismissed for the session).
    if (typeof requestApproveConsent === 'function') {
      requestApproveConsent(app.institution, app.firstName + ' ' + app.lastName, function() { commitApproveFromQueue(id); });
    } else {
  ...
  function commitApproveFromQueue(id) {
    ...
    showToast(app.firstName + ' ' + app.lastName + '’s application approved and forwarded to institution review.', 'success');
  }
